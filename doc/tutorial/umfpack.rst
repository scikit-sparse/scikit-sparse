...
The :mod:`sksparse.umfpack` module provides an interface to the SuiteSparse
`UMFPACK <umfpack_github_>`_ package, which computes basic linear algebra
...
:math:`P` and :math:`Q`, and row-scaling :math:`R`, such that:
...
:math:`Ax = b`, or :math:`A^{\top} x = b`.

The :mod:`.umfpack` module exposes most of the capabilities of the UMFPACK
package including:
...
.. _umfpack_github: https://github.com/DrTimothyAldenDavis/SuiteSparse/tree/dev/UMFPACK
...
See the :ref:`example <umfpack-example>` below for a demonstration of the
...
.. literalinclude:: examples/umfpack_example.py
   :language: python

This figure shows the effect of AMD ordering that reduces the fill-in of the
LU factorization of a sparse matrix.

.. figure:: examples/umfpack_example.svg
   :alt: UMFPACK Example with AMD Ordering
   :align: center
   :width: 90%
...
manipulate the :class:`UMFFactor` object, the :mod:`.umfpack` module
provides the :func:`umf_factor` function that performs both the
...
factorization, the :func:`umf_solve` function can be used. This function
...
For more advanced usage, users can instantiate the :class:`UMFFactor`
class. This class can be instantiated directly using its constructor, or more
conveniently using the :func:`umf_factor` function.
...
factorization is then performed by calling the :meth:`.UMFFactor.factorize`
method.

The :func:`umf_factor` function performs both the symbolic analysis and the
numeric factorization in one step, and returns an instance of the
:class:`UMFFactor` class. The resulting :class:`UMFFactor` object can then be
used to solve linear systems using its :meth:`UMFFactor.solve` method. The
:meth:`UMFFactor.factorize` method can be called again to factor a new matrix
with the same sparsity pattern.

Customization and Statistics
++++++++++++++++++++++++++++

The :func:`umf_factor` accepts several optional parameters that control the
behavior of the factorization, such as the fill-reducing ordering method to
use. These options are stored in a :class:`UMFControl` object that can be
accessed via the :attr:`UMFFactor.control` attribute. The :class:`UMFControl`
object can also be instantiated directly to customize the factorization
options before passing it to the :class:`UMFFactor` constructor or the
:func:`umf_factor` function. Any options not explicitly set by the user will
be given the UMFPACK default values.

After a factorization is computed, statistics about the factorization can be
accessed via the :attr:`UMFFactor.info` attribute, which is an instance of
the :class:`UMFInfo` class. This object contains various performance metrics
...
type :exc:`UMFPACKWarning`. The module will also issue
...
of type :exc:`UMFPACKError` or an appropriate subclass. See the
:ref:`umfpack-exceptions` for details.
